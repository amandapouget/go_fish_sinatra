class Spinach::Features::PlayGameWithRobots < Spinach::FeatureSteps
  ...
  step 'the game has started with robots' do
    game_with_three_robots_each_has_one_different_card
  end
  ...
  step 'I ask my first opponent for cards he does not have' do
    visit_player_page
    make_my_request
    expect(page).to have_content /#{@me_player.name} asked \S* for/
  end

  step 'turn advances correctly and informs me of play' do
    expect(page).to have_content /#{@first_opponent.name}'s turn/
    expect(page).to have_content /#{@first_opponent.name} asked \S* for/
    expect(page).to have_content /#{@second_opponent.name}'s turn/
