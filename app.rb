...
def make_game(num_players)
  if PENDING_USERS[num_players].size >= num_players
    users_for_game = PENDING_USERS[num_players].slice!(0, PENDING_USERS[num_players].size)
...
post '/subscribed' do # this post tells the server the 'wait' page has loaded!
  match = make_game(params["num_players"].to_i)
...
post '/:match_id/card_request' do
  ...
  rank = params["rank"]
  if !match.over && match.game.next_turn == player
